...
public interface RequestRepository extends JpaRepository<Request, Integer> {
    ...
    @Query("select r from Request r where r.state = ?1 and r.user_requester.service = ?2")
    List<Request> findByStateAndUser_requesterService(int state, Service service);
}